...
template formatIt*(T: type, body: untyped) {.dirty.} =
  template chroniclesFormatItIMPL*(it: T): auto = body

template chroniclesExpandItIMPL*(record: auto, field: static string, value: auto) =
...
macro expandIt*(T: type, expandedProps: untyped): untyped =
  ...
  for prop in expandedProps:
    ...
    setPropertyCalls.add newCall("setProperty", record, key, newCall(chroniclesFormatItIMPL, value))

  result = quote do:
    template chroniclesExpandItIMPL*(`record`: auto, `it_name`: static string, `it`: `T`) =
      mixin setProperty, chroniclesFormatItIMPL
      `setPropertyCalls`
